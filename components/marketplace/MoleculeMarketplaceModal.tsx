...
export const MoleculeMarketplaceModal: React.FC = () => {
  ...
  const handleLoadPreset = (preset: MoleculePreset) => {
    try {
      ...
      showToast(`Loaded ${preset.name} (${preset.formula}) into 3D Workspace!`);
    } catch (err: any) {
      ...
      <div className="relative flex h-[85vh] w-full max-w-5xl flex-col rounded-xl border border-slate-800 bg-slate-900/95 shadow-2xl overflow-hidden text-slate-100">
        ...
        <div className="flex shrink-0 items-center justify-between border-b border-slate-800 bg-slate-950 px-6 py-4">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-gradient-to-tr from-purple-600 via-pink-600 to-amber-500 shadow-lg shadow-purple-900/30">
              <ShoppingBag className="h-5 w-5 text-white" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-lg font-extrabold tracking-tight bg-gradient-to-r from-purple-300 via-pink-200 to-amber-200 bg-clip-text text-transparent">
                  Molecule Marketplace
                </h2>
                <span className="rounded-full bg-purple-950 px-2.5 py-0.5 text-[10px] font-bold text-purple-300 border border-purple-800">
                  Pre-built 3D Library
                </span>
              </div>
              <p className="text-xs text-slate-400">
                Instant VSEPR-relaxed 3D structures — Load methane, isobutane, benzene & more without rebuilding
              </p>
            </div>
          </div>
        ...
        <div className="flex shrink-0 flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 border-b border-slate-800/80 bg-slate-900/60 p-4">
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full rounded-lg border border-slate-800 bg-slate-950 pl-9 pr-4 py-2 text-xs text-slate-200 placeholder-slate-500 focus:border-purple-500 focus:outline-none focus:ring-1 focus:ring-purple-500 transition"
            />
            {searchQuery && (
              <button
        ...
        <div className="flex-1 overflow-y-auto p-5 no-scrollbar">
          {filteredPresets.length === 0 ? (
            <div className="flex h-full flex-col items-center justify-center text-center p-8">
              ...
              <p className="text-xs text-slate-600 mt-1">Try searching for Methane, Isobutane, Water, CO2, Benzene or clear filters.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredPresets.map((preset) => (
                <div
                  key={preset.id}
                  className="group relative flex flex-col justify-between rounded-xl border border-slate-800 bg-slate-950/80 p-4 transition-all duration-200 hover:border-purple-500/60 hover:shadow-xl hover:shadow-purple-950/40 hover:-translate-y-0.5"
                >
                  {/* Preset Header */}
                  <div>
                    <div className="flex items-start justify-between gap-2 mb-1.5">
                      <div>
                        <h3 className="font-extrabold text-sm text-slate-100 group-hover:text-purple-300 transition">
                          {preset.name}
                        </h3>
                      ...
                      <span className="rounded-lg bg-gradient-to-tr from-purple-950 to-indigo-900 border border-purple-700/60 px-2.5 py-1 font-mono text-xs font-bold text-purple-200 shadow">
                        {preset.formula}
                      </span>
                    </div>
                  ...
                  <div>
                    <div className="grid grid-cols-2 gap-1.5 mb-3 text-[10px] font-mono">
                      ...
                      <div className="rounded bg-slate-900/90 border border-slate-800/80 p-1.5 flex items-center justify-between">
                        ...
                        <span className="font-semibold text-purple-300">{preset.pointGroup}</span>
                      </div>
                    ...
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-2 rounded-lg bg-gradient-to-r from-purple-600 via-pink-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 py-2 text-xs font-bold text-white shadow-md shadow-purple-950/50 transition active:scale-[0.98]"
                    >
