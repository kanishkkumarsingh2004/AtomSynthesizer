...
export class AutoBondEngine {
  public static autoBondMolecule(
    ...
    // 1. Break hyper-bonded Hydrogen or excessively distant bonds
    for (const bond of graph.getAllBonds()) {
      ...
      if (autoBreakDistantBonds) {
        ...
        const breakThreshold = (radA + radB) * 2.3;
    ...
    // 2. Check candidate atom pairs for auto-bonding
    for (let i = 0; i < atoms.length; i++) {
      for (let j = i + 1; j < atoms.length; j++) {
        ...
        // STRICT HARD BLOCK: Hydrogen (H) or Halogens (F, Cl, Br, I) cannot exceed 1 bond
        ...
        const dist = distance(atomA.position, atomB.position);
        const maxDist = rSingle * toleranceRatio; // Generous distance threshold (~2.4 Å)
        ...
        if (dist <= maxDist && dist > 0.05) {
          ...
          if (atomA.atomicNumber !== 1 && atomB.atomicNumber !== 1) {
            // Triple bond condition: both atoms need >= 3 bonds and distance is close
            if (neededA >= 3 && neededB >= 3 && dist <= rSingle * 1.2) {
              order = 3;
              type = 'TRIPLE';
            }
            // Double bond condition: both atoms need >= 2 bonds
            else if (neededA >= 2 && neededB >= 2 && dist <= rSingle * 1.45) {
              order = 2;
              type = 'DOUBLE';
            }
          }
